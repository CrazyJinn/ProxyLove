...
description: |
  推进 Section 图节点的定稿段：读 structurer 的章级设计简报 + outliner 的本节提纲 outline.md → 创作逐句对话 → 产出节级 台词.ink（人读/人改的唯一定稿格式，ink 方言：场景块标记「=== <scene_block_id> <Scene 名>（<时段>）」、说话行「角色名:台词」**不写 [表情] 标注**——专注台词创作，演出层（表情/立绘变体）与台词分离、旁白行「旁白:叙述」、ink 原生选择「* 选项 -> 去向」/分支「= label」/结局「-> END # ending:」）→ 兜底建 SecScript 产物节点（SecOutline-[:produces]->SecScript）写 script_path + status=10（定稿待审，直写不经 submit）。
  台词.ink 是正式定稿格式（方言规范见 references/ink方言规范.md）：dashboard 定稿审渲染它；审批通过（sc=11）后由 section-voice-publisher 第一步拆分进图（script_splitter.py parse_ink → 逐句 LineAudio 节点）再配音。BGM 不归本 skill（BgmTrack 由 scene-design 编排 bgm-designer 管理）。
...
#### 台词.ink 格式规范（正式定稿格式，ink 方言，机器可解析）

> 完整方言规范（行型表/选择行细则/硬约束/与严格 ink 的偏离）见 [references/ink方言规范.md](references/ink方言规范.md)——本节为创作侧速览。
...
* <选项文本> -> <去向：label 名 / scene_block_id / END # ending: <kind>>
* <选项文本> -> <去向>
...
1. **场景块标记**：`=== <scene_block_id> <Scene 名>（<时段>）`（knot 风格）——scene_block_id 照搬 structurer 预分配值（如 `s00_酒店`，章内唯一），Scene 名/时段照搬提纲契约值。括号时段仅供人读（运行时时段以 Scene.time_of_day 为准，拆分进图不存）。
2. **说话行**：`角色名:台词`——一行一句，冒号用**半角**，角色名用 Character.name 原名（「旁白」是保留字）。**禁止写 `[表情]` 标注**（如 `[微笑]`）：本 skill 专注台词创作，演出层（立绘变体选择）与台词**彻底分离**——由 section-voice-publisher 配音判断期为每句 say 行按台词氛围选定（`LineAudio-[:uses]->StandingIllustration` 边），定稿不承载任何演出标注。**残留 `[表情]` 会在拆分时报错**（parse_ink 拒绝带方括号的说话行，报行号+原文）。pos 不进定稿（拆分时定）。
...
5. **选择行**（ink 原生 `*`/`+`）：`* 选项文本 -> 去向`——单行，**禁止缩进的选项正文块**（分支正文一律写在 `= <label>` 之后）。去向 = label 名 / scene_block_id / `END # ending: <kind>`（直达结局必须带 ending tag——仅拓扑记录）。**选择行暂不进图**（choice 建模后续设计）——解析器整行跳过（含行尾 tag）。
6. **分支行**（stitch）：`= <label 名>` 标示分支正文起点（进图 op=label）。
7. **结局行**：`-> END # ending: <kind>——<落点一句话>`（进图 op=ending；注意 ink 中 `#` 是 tag 非注释，行首 `#` 非法）。
8. **覆盖完整**：提纲全部场景段、分支、结局都要写，不得增删场景/分支。
9. **情感递进**：每个场景内部情绪有起伏（不是平铺），对齐设计简报情感弧线的该段位置。
10. **机器可解析（硬约束）**：台词.ink 是 section-voice-publisher 拆分进图（`script_splitter.py parse_ink`）的输入——行型严格依赖行首标记（`===`/`=`/`* `/`+ `/`-> `/`//`/`旁白:`/`环境音:`/`角色名:`），**台词正文中不得出现这些行首前缀**（换行重写）；无法解析的行会在拆分时报错（行号+原文），拆分前必须修 ink。「旁白」「环境音」均为保留字，角色不得使用这两个名字。裸 divert（`-> xxx` 非结局式）不支持（jump 暂无图行建模，写了报错）。
11. **Write 落盘**：`25_剧本/chapter<NN>_<章概述>/sec<MM>_<节概述>/台词.ink`（与该节 outline.md 同目录）。`SecScript.script_path` 指向此 `.ink` 路径。Write 自动创建章/节目录。
