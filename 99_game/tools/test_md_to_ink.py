...
锁定两件事：
  ...
  ② 3 个真实 台词.md（chapter00 序章，2026-09-02 迁移时全文内嵌为 fixture——源文件
     迁移后已删）+ 合成样例：parse_ink(convert(md)) == parse_md(md) 深相等——
     行文本逐字保真的机器证明（对齐签名不变 → 存量图行全 keep）。
...
def test_convert_synthetic_rows_deep_equal(tmp_path):
    md = tmp_path / "synth.md"
    md.write_text(SYNTH, encoding="utf-8")
    ink_text = md_to_ink.convert_file(md)
    ink = tmp_path / "synth.ink"
    ink.write_text(ink_text, encoding="utf-8")
